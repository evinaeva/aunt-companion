...
async def voice_on_handler(message: Message, db_conn: aiosqlite.Connection, default_tts_voice: str) -> None:
    await _set_voice_toggle(message, db_conn, enabled=True, default_tts_voice=default_tts_voice)


@router.message(Command("voice_off"))
async def voice_off_handler(message: Message, db_conn: aiosqlite.Connection, default_tts_voice: str) -> None:
    await _set_voice_toggle(message, db_conn, enabled=False, default_tts_voice=default_tts_voice)
...
async def voice_message_handler(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(dir=tmp_dir) as workdir:
            ...
            logger.info("Voice converted by ffmpeg", extra={"telegram_user_id": from_user.id})

            try:
                transcript = await stt_adapter.transcribe(str(wav_path))
                logger.info("STT ok", extra={"telegram_user_id": from_user.id, "transcript_len": len(transcript)})
            except Exception:
                logger.exception("STT request failed")
                await message.answer("Не понял голос")
                return
        ...
        should_voice_reply = bool(settings.voice_enabled) if settings else False
        if should_voice_reply:
            try:
                voice_path = await tts_adapter.synthesize(reply_text)
                voice_data = Path(voice_path).read_bytes()
                await message.answer_voice(BufferedInputFile(voice_data, filename="reply.ogg"))
                logger.info("TTS ok", extra={"telegram_user_id": from_user.id, "voice_path": voice_path})
            except Exception:
                logger.exception("TTS request failed")
                logger.info("Fallback to text after TTS failure", extra={"telegram_user_id": from_user.id})
                await message.answer(reply_text)
            return
...
async def _set_voice_toggle(message: Message, db_conn: aiosqlite.Connection, *, enabled: bool, default_tts_voice: str) -> None:
    from_user = message.from_user
    if from_user is None:
        await message.answer("Не удалось определить пользователя Telegram.")
        return
    ...
    await settings_repo.set_voice_enabled(user.id, enabled=enabled, default_tts_voice=default_tts_voice)
